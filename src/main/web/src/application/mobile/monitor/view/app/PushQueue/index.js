...
class Index extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({type: "appModel/loadAllApp"}).then(() => {
      dispatch({type: 'appModel/querySieIfList2'}).then(() => {
        ...
        dispatch({type: "appModel/queryAllNumber2", payload: {appList, sieIfList, type: 'push-queue'}}).then(() => {
  ...
  getContent(selectNumber) {
    ...
    let status = "阻塞总数"
    let color = "red"
    let pictureUrl = "./application/mobile/push_queue_circle.png"
    if (selectNumber && selectBar === "app") {
      return (selectNumber.map((item, index) =>
        <div key={index} onClick={() => {
          this.clickData(item)
        }}>
          <DataInfo name={item.app.name} status={status} current={item.app.id} time={item.total}
  ...
  render() {
    ...
    const {allNumber2} = this.props.appModel
    let allNumber = allNumber2
